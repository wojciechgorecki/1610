namespace SmallStacker.SAP
{
    ...
    public class DriverSAP
    {
       ...
       // public static string _mode = "PROD";
        ...
        public bool BackContainerBuffor(string userName, string FVI_VLPLA, char FVI_NO_LAGP, BackContainerBufforReturn bc)
        {
            bool success = false;
            try
            {
                ...
                Z_MFCS_DEL_FROM_ST.SetValue("FVI_PERNR", PerNBR);
                ...
                try
                {
                    DatabaseController.AddingLogData(FVI_VLPLA, "Usuwanie kontenera z buforu stacji ", PerNBR, userName, "Z_MFCS_DEL_FROM_ST", bc.ReturnCode);
                }
                catch (Exception ex)
                {
        ...
        /// <returns>Zwraca prawde/falsz zaleznie od tego czy udalo sie wykonac akcje.</returns>
        public bool ContainerInfo(string userName, string FVI_VLPLA, char FVI_NO_LAGP, ContainerInfoReturn _containerInfoReturn) // idUser - HR id
        {
            bool success = false;
            try
            {
                ...
                // FVI_PERNR - id użytkownika (NUM8)
                z_mfcs_info.SetValue("FVI_PERNR", PerNBR);
                ...
                try
                {
                    DatabaseController.AddingLogData(FVI_VLPLA, "Informacje o kontenerze ", PerNBR, userName, "Z_MFCS_INFO", _containerInfoReturn.ReturnCode);
                }
                catch (Exception ex)
                {
                    Messenger.Default.Send(new LogMessage(ex.Message, LogType.DATABASELOG),"Log");
        ...
        /// <returns>Zwraca prawde/falsz zaleznie od tego czy udalo sie wykonac akcje.</returns>
        public int Z_MFCS_SEND_HT(string userName, string FVI_VLPLA, char FVI_EMPTY, string FVI_DEST, char FVI_NO_LAGP, int FVI_PRIORITY, out string error)
        {
            error = string.Empty;
            int ret = -1;
            try
            {
                ...
                // FVI_PERNR - id użytkownika (NUM8)
                z_MFCS_SEND_HT.SetValue("FVI_PERNR", PerNBR);
                ...
                try
                {
                    DatabaseController.AddingLogData(FVI_VLPLA, "Pobranie kontenera " + FVI_DEST + " ", PerNBR, userName, "Z_MFCS_SEND_HT", ret);
                }
                catch (Exception ex)
                {
                    Messenger.Default.Send(new LogMessage(ex.Message, LogType.DATABASELOG),"Log");
